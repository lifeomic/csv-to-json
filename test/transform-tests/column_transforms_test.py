...
def test_default():
    ...
    table = column_transforms.default(
        sub_map, table, 'generic_string', 'operation_col')
...
def test_default_uuid():
    ...
    table = column_transforms.default(
        sub_map, table, 'source_col', 'operation_col')
    assert(not pd.isnull(table['operation_col'][0])
           and len(table['operation_col'][0]) == 36)
...
def test_time_delta():
    ...
    assert(table['base_time'][0] ==
           '1988-7-8 07:22' and table['day_shift'][0] == '5')
...
def test_trim_whitespace():
    ...
    table = column_transforms.trim_whitespace(
        {}, table, 'whitespace_words', 'operation_col')
    assert(list(table['whitespace_words']) ==
           ['     wordone      ', 'wordtwo'])
...
def test_uppercase():
    ...
    table = column_transforms.uppercase(
        {}, table, 'lowercase_words', 'operation_col')
...
def test_lowercase():
    ...
    table = column_transforms.lowercase(
        {}, table, 'uppercase_words', 'operation_col')
...
def test_format_date():
    ...
    table = column_transforms.format_date(
        {}, table, 'unformatted_date', 'operation_col')
...
def test_string_concatenation():
    ...
    table['operation_col'] = table['generic_string']
    table = column_transforms.string_concatenation(
        sub_map, table, 'generic_string', 'operation_col')
